...
RETURN = '''
defaults:
  description: Whether results are defaults
  type: bool
  returned: success
prefs:
  description: Preferences for Qubes VM
  returned: success
  type: dict
  contains:
    audiovm:
...
def main():
  module = AnsibleModule(
    ...
    required_if=[('defaults', False, ('name',), True)],
    mutually_exclusive=[('name', 'defaults')]
  )
  prefs = QubesPrefs(module.params["defaults"], target=module.params["name"])
  return module.exit_json(
    defaults=module.params["defaults"],
    prefs=prefs.get_properties()
  )
